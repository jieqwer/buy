...
$pieces2 = explode(",", $row[24]);//手机展示图


for($y=0;$y<count($pieces1);$y++){
    $z=$y+1;
    if($y==$im){
        echo "<form class='layui-form' action='img_update2_check.php?id=$id&im=$im' method='post' enctype='multipart/form-data'>
    <div class='layui-form-item'><label class='layui-form-label'>第 $z 张图</label>
    ...
    <label class='layui-form-label' style='line-height: 60px'>宣传词</label> 
        <div class='layui-input-inline' style='width:600px;float: left;margin-top: 20px;'>
            <input type='text'  style='line-height: 40px'  name='say$im'  value='$pieces2[$im]'  autocomplete='off' class='layui-input'>   
        </div>
    </div>
